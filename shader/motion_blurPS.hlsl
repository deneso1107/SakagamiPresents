...
//cbuffer PostProcessBuffer : register(b0) // 定数バッファは16バイト境界に揃える
//{
//    float blurStrength;
...
//    float _padding[2]; // ← ここで16バイト境界に揃える（合計32バイト）
//};
...
//    float2 toCenter = -normalize(fromCenter); // 中心に向かう方向
    
//    float edgeFactor = smoothstep(0.0, 0.7, distance);
    
//    // === モーションブラー（強化版） ===
//    float4 blurColor = float4(0, 0, 0, 0);
//    int blurSamples = 10; // サンプル数を増やす
...
//        float offset = blurStrength * edgeFactor * t * 0.08; // 0.03 → 0.08に増加
...
//    //    float angleSegment = floor(angle * lineCount / (3.14159 * 2.0));

//    //    float random1 = frac(sin(angleSegment * 12.9898) * 43758.5453);
//    //    float random2 = frac(sin(angleSegment * 78.233) * 43758.5453);

//    //    float lineWidth = 0.02 + random1 * 0.04;
//    //    float lineOffset = random2 * 0.3;
//    //    float linePattern = frac(angle / (3.14159 * 2.0) * lineCount + lineOffset);

//    //// --- 外方向への流れ追加 ---
//    //// 中心から外方向へ距離に応じて時間でスクロール
//    //    float distanceOffset = random2 * 0.5;
//    //    float flow = frac((distance + distanceOffset) * 1.8 + time * speedLineSpeed);
//    //    float flowPattern = smoothstep(0.0, 0.06, flow) * (1.0 - smoothstep(0.25, 0.35, flow));
//    //// -----------------------------

//    //    speedline = smoothstep(lineWidth, 0.0, abs(linePattern - 0.5) * 2.0);

//    //    if (random1 < 0.25)
//    //        speedline = 0.0;

//    //    float fade = smoothstep(0.2, 0.9, distance);

//    //// 流れ効果を合成
//    //    speedline *= fade * blurStrength * 3.5 * flowPattern;
//    //}
...
//    // ブラーの混合率を上げる
//    float blurAmount = edgeFactor * blurStrength * 0.8; // 0.5 → 0.8
//    float3 finalColor = lerp(sceneColor.rgb, blurColor.rgb, blurAmount);
    
//    // スピードラインを追加
//    finalColor += float3(speedline, speedline, speedline);
