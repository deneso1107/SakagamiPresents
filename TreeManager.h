...
struct TreeCollisionResult
{
    bool hit;           // 衝突したか
    Vector3 normal;     // 衝突法線（木→プレイヤー方向）
...
private:
    ...
    static constexpr float TREE_COLLISION_RADIUS = 0.5f; // 木1本の当たり半径（要調整）
...
public:
    ...
    // 衝突判定（プレイヤー位置と半径を渡す）
    TreeCollisionResult CheckCollision(const Vector3& playerPos, float playerRadius) const;

    // 衝突応答（速度ベクトルを書き換えて返す）
    static Vector3 ResolveCollision(
        const TreeCollisionResult& result,
        const Vector3& velocity,
        float restitution,   // 反発係数 0.0〜1.0
        float friction       // 摩擦係数 0.0〜1.0
    );
